...
2. **第一期**:先做完 **M6+M7**,再开 M8 电梯;按计划自主推进。
3. **生图**:Dofun `gpt-image-2`;图进 `public/modules/`。
4. **未完成模组文案**:统一用 **「即将开发」**(不是「即将开放」),可进简介页,不可开玩。
5. **每个模组必须有**:简单故事背景 + 玩法介绍(主页卡/详情页可见;可玩模组开局前也能看到)。
6. **验收标准**:门禁全绿 + 真人路径无糟糕体验(主页可读、找猫可玩、preview 不误导、移动端无横向溢出)。
